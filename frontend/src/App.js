...
class App extends Component {
  ...
  login() {
    if (this.state.username.localeCompare("error:not logged in") === 0) {
      ...
      if (temp != null) {
        ...
        axios.get(`http://localhost:80/getuser`, {
          params: {
            code: temp
          }
        })
      ...
      if (this.state.password.localeCompare("error:not logged in") === 0) { }
        ...
        if (this.state.username.localeCompare(". . .") === 0) {
          ...
          if (this.state.contest_fetched === 0) {
            ...
            axios.get(`http://localhost:80/contestlist`, {
              params: {
    ...
    if (this.state.username.localeCompare("error:not logged in") === 0) {
      ...
      if (this.state.username.localeCompare(". . .") === 0) {
        ...
        if (this.state.contest_code.localeCompare(code) === 0) {
          ...
          axios.get(`http://localhost:80/contest`, {
            params: {
              username: this.state.username,
              password: this.state.password,
              contest: code
  ...
  getRanklist(code) {
    ...
    axios.get(`http://localhost:80/ranklist`, {
      params: {
        username: this.state.username,
        password: this.state.password,
        contest: code
  ...
  getSubmissions(code) {
    ...
    axios.get(`http://localhost:80/submission`, {
      params: {
        username: this.state.username,
        password: this.state.password,
    ...
    if (this.state.username.localeCompare("error:not logged in") === 0) {
      ...
      if (this.state.username.localeCompare(". . .") === 0) {
        ...
        if (this.state.problem_code.localeCompare(code) === 0) {
          ...
          axios.get(`http://localhost:80/problem`, {
            params: {
              username: this.state.username,
              password: this.state.password,
              contest: this.state.contest_code,
        ...
        <Router>
          ...
          <Switch>
            ...
            <Route exact path="/submit">
              <Submit problem_details={this.state.problem_details} problem_code={this.state.problem_code} />
            </Route>
